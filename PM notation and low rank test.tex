...
We evaluate the proposed Laplace-EM algorithm on a synthetic dataset drawn from the model specified in Chapter~3. The data-generating process uses $J = 30$ groups, $N = 800$ total observations, $Q = 50$ categories, $K = 2$ latent factors, $P = 2$ covariates, and $\sigma^2_{\mathrm{true}} = 0.3$. The true factor loading matrix $\mathbf{B}^*$ is constructed in positive lower-triangular (PLT) form with diagonal entries drawn from $\mathrm{Uniform}(0.5,\,1.2)$ and sub-diagonal entries from $\mathcal{N}(0,\,0.36)$. Group-level factors are generated as $\mathbf{f}_j^* \sim \mathcal{N}(0, I_K)$ independently.
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.48\textwidth}
        ...
        \caption{Random effect Recovery}
        \label{fig:Basictest2}
    \end{subfigure}
\end{figure}

Figure~\ref{fig:Basictest1} displays the Laplace log-evidence $\log\tilde{p}(\bm{y}\mid\bm{\theta})$ over EM iterations. The sequence is strictly monotone throughout, with zero violations of the monotonicity criterion (we set the threshold to be $10^{-6}$), consistent with the theoretical guarantee that each EM cycle increases the observed-data marginal log-likelihood under the Laplace approximation.

Figure~\ref{fig:Basictest2} shows side-by-side heatmaps of $\bm{\lambda}_{j}^*$ and $\hat{\bm{\lambda}}_{j}$. The PLT structure and the dominant loading pattern are faithfully recovered. The group-level random effects $\{\bm{\lambda}_j\}$ are recovered with mean Pearson correlation across all $J=30$ groups (Figure~\ref{fig: Basictest4}), with no groups yielding negative correlations, confirming that the Laplace E-step reliably locates the posterior mode even under the high-dimensional multinomial likelihood.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Rplottest4.pdf}
    \caption{$\mathbf{f}_{j}$ Recovery via Procrustes alignment}
    \label{fig: Basictest4}
\end{figure}

Factor scores are identifiable only up to an orthogonal rotation, so Procrustes alignment is assessed in $\mathbf{f}_{j}$ check. Let the orthogonal matrix $\mathbf{R} = \mathbf{UV}^\top$ where $\mathbf{F}^* \hat{\mathbf{F}}^\top = \mathbf{UDV}^\top$, then evaluate $\hat{\mathbf{F}}_{\mathrm{aligned}} = \mathbf{R}\hat{\mathbf{F}}$. After alignment, Factor~1 achieves $r = 0.985$ and Factor~2 achieves $r = 0.997$ (Figure~\ref{fig:Basictest4}), confirming that the latent factor structure is correctly identified. The relative Frobenius error of $0.34$ reflects systematic posterior shrinkage inherent to the Bayes linear estimator \ref{eq: Bayeslinearestimator}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Rplottest3.pdf}
    \caption{Boxplots of Parameters' Estimation}
    \label{fig: Basictest3}
\end{figure}

Figure~\ref{fig: Basictest3} reports results across 20 independent replications under the same data-generating process with different random seeds. The subspace distance $d_{\mathrm{sub}}(\hat{\mathbf{B}},\mathbf{B}^*)$, the idiosyncratic variance estimate $\hat{\sigma}^2$, and the mean group correlation $\bar{r}$ are all stable across seeds, demonstrating that the algorithm is insensitive to random initialization and data realization.

\section{Real Data Estimation}
In this section, we apply the proposed algorithm to the dataset ``dietswap" in microbiome R package \citep{Lahti2017}. The study recruited 38 healthy subjects from two populations, Finnish and African, and collected multiple stool samples from each subject across timepoints during a controlled diet-swap intervention. This yields a grouped multivariate count structure with $I = 222$ observations nested within $J = 38$ subjects. The response is a vector of read counts over $Q_{\mathrm{raw}} = 130$ operational taxonomic units (OTUs). The multiple observations in this dataset share a common group-level random effect, which satisfies the structural assumption of our model.

We retain taxa present in at least 5 samples to avoid inflation. The total read count per sample, $y_{ij\cdot} = \sum_q y_{ijq}$, ranges from 1776 to 28883. A single binary covariate nationality ($x_i = 1$ for African, $x_i = 0$ for Finnish) is included, giving $P = 2$ (intercept plus nationality). Groups are defined by subject identity, so all samples from the same subject share a common group-level random effect $\bm{\lambda}_j$.

We fit the model with $K = \{2,3,4\}$  latent factors. $K=3$ has the best performance that converged at iteration 49 (with tolerance $10^{-4}$) out of a maximum of 120 iterations, while $K=2$ and $K=4$ do not converged in 120 iterations. Figure~\ref{fig:microtest2} shows the overall posterior predictive pseudo-$R^2$ computed as the squared Pearson correlation between observed and predicted counts across all samples and taxa, which is 0.7846. Figure~\ref{fig:microtest1} shows the separation...

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Rplotmicrotestoverallpseudo.pdf}
    \caption{Overall Pseudo $R^2$ when $K=3$}
    \label{fig:microtest2}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Rplotmicrofactorscores.pdf}
    \caption{Factor Scores while $K=3$}
    \label{fig:microtest1}
\end{figure}
